...
const Testimonials = () => {
    ...
    useEffect(() => {
        fetch('https://online-restaurant-server.vercel.app/review')
            .then(res => res.json())
            .then(data => setReviews(data))
    }, [])

    return (
        <div className="my-8 max-w-sm lg:max-w-3xl mx-auto ">
            <Heading subheading={'---What Our Clients Say---'} heading={'TESTIMONIALS'} />
            <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
                {
                    reviews.map(review => <>
                        <SwiperSlide key={review._id}>
                            <div className="p-14 flex flex-col justify-center items-center gap-y-3">
                                ...
                                <div className="text-center text-sm">
                                    <p>{review.details}</p>
                                    <p className="text-xl font-semibold text-[#CD9003]">{review.name}</p>
                                </div>
                            </div>
                        </SwiperSlide>
                    </>)
